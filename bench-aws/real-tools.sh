...
record $N "checkpoint-500mb" "bytehaul" "500MB"

# rsync -avz (THE default for most teams)
nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); rsync -avz -e \"ssh \$K\" /tmp/model.pt ec2-user@$IPB:/tmp/rv/ >/dev/null 2>&1; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "checkpoint-500mb" "rsync-avz" "500MB"

# scp -C (scp with compression)
nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); scp -C -q \$K /tmp/model.pt ec2-user@$IPB:/tmp/rv/b; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "checkpoint-500mb" "scp-C" "500MB"

# tar | ssh (no compression, fastest TCP pipe)
nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); tar cf - -C /tmp model.pt | ssh \$K ec2-user@$IPB 'tar xf - -C /tmp/rv/'; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "checkpoint-500mb" "tar-ssh" "500MB"

# tar | pigz | ssh (parallel-compressed pipe)
nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); tar cf - -C /tmp model.pt | pigz -1 | ssh \$K ec2-user@$IPB 'pigz -d | tar xf - -C /tmp/rv/'; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "checkpoint-500mb" "tar-pigz-ssh" "500MB"

# rclone over SFTP
nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); rclone copy /tmp/model.pt gpu:/tmp/rv/ 2>/dev/null; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "checkpoint-500mb" "rclone-sftp" "500MB"
...
record $N "shards-600" "bytehaul" "600x200KB"

nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); rsync -avz -e \"ssh \$K\" /tmp/shards ec2-user@$IPB:/tmp/rv/ >/dev/null 2>&1; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "shards-600" "rsync-avz" "600x200KB"

nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); tar cf - -C /tmp shards | ssh \$K ec2-user@$IPB 'tar xf - -C /tmp/rv/'; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "shards-600" "tar-ssh" "600x200KB"

nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); rclone copy /tmp/shards gpu:/tmp/rv/shards 2>/dev/null; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "shards-600" "rclone-sftp" "600x200KB"
...
record $N "large-model-5gb" "bytehaul" "5GB"

nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); rsync -av -e \"ssh \$K\" /tmp/big_model.pt ec2-user@$IPB:/tmp/rv/ >/dev/null 2>&1; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "large-model-5gb" "rsync-av" "5GB"

nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); tar cf - -C /tmp big_model.pt | ssh \$K ec2-user@$IPB 'tar xf - -C /tmp/rv/'; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "large-model-5gb" "tar-ssh" "5GB"

nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); scp -q \$K /tmp/big_model.pt ec2-user@$IPB:/tmp/rv/b; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "large-model-5gb" "scp" "5GB"
...
record $N "logs-compressed" "bytehaul-zstd" "200x-logs"

nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); rsync -avz -e \"ssh \$K\" /tmp/logs ec2-user@$IPB:/tmp/rv/ >/dev/null 2>&1; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "logs-compressed" "rsync-avz" "200x-logs"

nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); tar cf - -C /tmp logs | pigz -1 | ssh \$K ec2-user@$IPB 'pigz -d | tar xf - -C /tmp/rv/'; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"") || T=0
record $N "logs-compressed" "tar-pigz-ssh" "200x-logs"
