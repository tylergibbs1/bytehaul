...
nx; T=$(bh "" "/tmp/checkpoint_step1000.pt");  record $N "checkpoint-500mb" "bytehaul" "500MB-checkpoint" "$T" 500
nx; T=$(sc "/tmp/checkpoint_step1000.pt");      record $N "checkpoint-500mb" "scp" "500MB-checkpoint" "$T" 500
nx; T=$(bh "--compress" "/tmp/checkpoint_step1000.pt"); record $N "checkpoint-500mb-compressed" "bytehaul-zstd" "500MB-checkpoint" "$T" 500

# ═══ 4-6: Dataset staging (600 files, ~120MB) ═══
log "═══ 4-6: Dataset staging (600 shards) ═══"
nx; T=$(bh "-r" "/tmp/dataset");                record $N "dataset-600files" "bytehaul" "600x200KB" "$T" 117
nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); rsync -a -e \"ssh \$K\" /tmp/dataset ec2-user@$IPB:/tmp/rv/; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"")
record $N "dataset-600files" "rsync" "600x200KB" "$T" 117
nx; T=$(R "K='$RK'; ssh \$K ec2-user@$IPB 'rm -rf /tmp/rv; mkdir -p /tmp/rv'; S=\$(python3 -c 'import time; print(time.time())'); tar cf - /tmp/dataset 2>/dev/null | ssh \$K ec2-user@$IPB 'tar xf - -C /tmp/rv/' 2>/dev/null; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"")
...
nx; T=$(bh_keep "--delta" "/tmp/checkpoint_step1000.pt"); record $N "delta-sync-10pct" "bytehaul-delta" "500MB-10pct-changed" "$T" 500
# Full re-send for comparison
nx; T=$(bh "" "/tmp/checkpoint_step1000.pt");          record $N "full-resend" "bytehaul" "500MB-full" "$T" 500
nx; T=$(sc "/tmp/checkpoint_step1000.pt");              record $N "full-resend" "scp" "500MB-full" "$T" 500
...
nx; T=$(R "K='$RK'; S=\$(python3 -c 'import time; print(time.time())'); scp -q \$K ec2-user@$IPB:/tmp/results.pt /tmp/pulled_scp.pt; E=\$(python3 -c 'import time; print(time.time())'); python3 -c \"print(\$E-\$S)\"")
...
nx; T=$(bh "-r --compress" "/tmp/ml_artifacts"); record $N "artifacts-compressed" "bytehaul-zstd" "configs+logs+metrics" "$T" 10
...
nx; T=$(bh "" "/tmp/checkpoint_step1000.pt"); record $N "fanout-single" "bytehaul" "500MB-single-dest" "$T" 500

# ═══ 15: Large model transfer (5GB) ═══
log "═══ 15: Large model transfer (5GB) ═══"
nx; T=$(bh "" "/tmp/large_model.pt"); record $N "large-model-5gb" "bytehaul" "5GB-model" "$T" 5120
# scp for comparison (will take a while)
nx; T=$(sc "/tmp/large_model.pt");    record $N "large-model-5gb" "scp" "5GB-model" "$T" 5120
